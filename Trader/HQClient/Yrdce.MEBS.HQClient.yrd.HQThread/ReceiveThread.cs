...
namespace Gnnt.MEBS.HQClient.gnnt.HQThread
{
	public class ReceiveThread : MYThread
	{
		...
		protected override void run()
		{
			try
			{
				...
				while (this.m_Client != null && !this.blnIsStopped)
				{
					...
					else
					{
						...
						{
							Logger.wirte(MsgType.Error, this.marketName + "接收线程发生IOException异常：" + ex2.ToString());
							if (this.m_Client != null)
							{
								if (this.m_Client.m_socket != null)
								{
						...
						{
							Logger.wirte(MsgType.Error, this.marketName + "接收线程发生InvalidDataException异常：" + ex3.ToString());
							if (this.m_Client != null)
							{
								if (this.m_Client.m_socket != null)
								{
						...
						{
							Logger.wirte(MsgType.Error, this.marketName + "接收线程发生ThreadAbortException异常：" + ex4.ToString());
							if (this.m_Client != null)
							{
								if (this.m_Client.m_socket != null)
								{
						...
						{
							Logger.wirte(MsgType.Error, this.marketName + "接收线程发生Exception异常：" + ex5.ToString());
							if (this.m_Client != null)
							{
								if (this.m_Client.m_socket != null)
								{
			...
			{
				Logger.wirte(MsgType.Error, this.marketName + (string)ex6.ExceptionState + "》》》》》》》》》》》》》》");
			}
			catch (Exception ex7)
			{
				Logger.wirte(MsgType.Error, this.marketName + "接收线程发生异常：" + ex7.ToString());
			}
		...
		private void ReceiveMarketDate(InputStreamConvert m_reader)
		{
			...
			{
				if ((this.setInfo.CurTimeMarketId == "" || this.setInfo.CurTimeMarketId == null) && (text == "00" || (!this.m_Client.m_htMarketData.ContainsKey("00") && text == this.fristMarketCode)))
				{
					this.m_Client.m_iTime = num2;
					if (this.m_Client.m_iDate == 0 || num != iDate)
					{
		...
		private void ReceiveQuoteList(InputStreamConvert reader)
		{
			...
			Logger.wirte(MsgType.Information, this.marketName + "本次更新数据条数=" + obj.Length);
		...
		private void ReceiveMinLineData(InputStreamConvert reader)
		{
			...
			Logger.wirte(MsgType.Information, "分时数据条数======" + obj.Length.ToString());
			...
			ProductData obj2;
			Monitor.Enter(obj2 = productData);
			try
			{
					...
					if (this.m_Client.m_htMarketData != null)
					{
						...
						try
						{
							if (enumerator.MoveNext())
							{
								...
								{
									if (((MyButton)this.buttonUtils.ButtonList[0]).Name != myButton.Name)
									{
										this.buttonUtils.InsertButton(0, myButton, this.buttonUtils.ButtonList);
									}
								}
