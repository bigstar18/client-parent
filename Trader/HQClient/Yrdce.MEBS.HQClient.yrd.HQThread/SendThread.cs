...
namespace Gnnt.MEBS.HQClient.gnnt.HQThread
{
	public class SendThread : MYThread
	{
				...
				if (packet != null)
				{
					...
					case 7:
						if ((DateTime.Now - this.datetimeBill).TotalMilliseconds >= 100.0)
						{
							...
							Logger.wirte(MsgType.Information, "------------" + packet.getCmd() + this.datetimeBill.ToLongTimeString());
							goto IL_206;
						}
						goto IL_206;
					case 10:
					...
					if (count > num)
					{
						for (int i = 0; i < count - num; i++)
						{
							...
							Logger.wirte(MsgType.Information, "AskForDataRemove:" + cMDVO.getCmd());
		...
		protected override void run()
		{
					...
					else
					{
						...
						if (count > 0)
						{
							...
							try
							{
								...
								default:
									Logger.wirte(MsgType.Information, "Send cmd:" + cMDVO.getCmd());
									break;
								}
			...
			{
				Logger.wirte(MsgType.Error, (string)ex2.ExceptionState);
			}
			catch (Exception ex3)
			{
				Logger.wirte(MsgType.Error, "发送线程run方法异常：" + ex3.Message);
			}
		}
