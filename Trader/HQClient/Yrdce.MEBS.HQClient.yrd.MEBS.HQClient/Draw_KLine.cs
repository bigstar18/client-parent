...
namespace Gnnt.MEBS.HQClient.gnnt.MEBS.HQClient
{
	public class Draw_KLine
	{
			...
			try
			{
				...
				if (this.m_rcPane[0].Width >= 0)
				{
					...
					for (int i = 0; i < 3; i++)
					{
						...
						{
							Logger.wirte(MsgType.Warning, "m_indicator[" + i + "]为空！");
						}
					}
		...
		private void MakeCycleData(int value)
		{
			...
			{
				if (7 == this.parent.m_hqClient.m_iKLineCycle)
				{
			...
			for (int i = 0; i < this.m_kData.Length; i++)
			{
				...
				if (kLineData != null)
				{
					...
					else
					{
						...
						{
							if (value.DayOfWeek == DayOfWeek.Tuesday)
							{
								num2 = 3;
							}
							else
							{
								if (value.DayOfWeek == DayOfWeek.Wednesday)
								{
									num2 = 4;
								}
								else
								{
									if (value.DayOfWeek == DayOfWeek.Thursday)
									{
										num2 = 5;
									}
									else
									{
										if (value.DayOfWeek == DayOfWeek.Friday)
										{
											num2 = 6;
										}
										else
		...
		private DateTime GetDateTime(long date, int iMin, bool isHistoryData)
		{
			...
			while (i < timeRange.Length)
			{
				...
				{
					if (result.CompareTo(dateTime) >= 0 && result.CompareTo(value) <= 0)
					{
				...
				using (Graphics graphics = Graphics.FromImage(bitmap))
				{
					...
					if (num < this.m_kData.Length)
					{
						...
						if (num > 0 && this.m_kData[num - 1] != null)
						{
							...
							{
								if (this.setInfo.AmountIncrease == "1")
								{
									num3 += font.Height;
		...
		private bool ChangePos(bool bIsLeft)
		{
			...
			else
			{
				...
				{
					if (this.m_iPos == this.m_pos.m_End - this.m_pos.m_Begin)
					{
						if (this.m_pos.m_Begin >= this.m_pos.m_MaxPos)
						{
							return false;
		...
		private DateTime GetDateTimePlus(long date, int iMin, bool isHistoryData)
		{
			...
			while (i < timeRange.Length)
			{
				...
				{
					if (dateTime.CompareTo(dateTime3) >= 0 && dateTime.CompareTo(value) <= 0)
					{
						num2 = i;
