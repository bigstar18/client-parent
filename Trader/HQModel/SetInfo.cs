...
// Type: Gnnt.MEBS.HQModel.SetInfo
...
namespace Gnnt.MEBS.HQModel
{
  public class SetInfo
  {
    ...
    {
      this.xmlDoc.SelectSingleNode("SetInfo").SelectSingleNode(key).InnerText = value;
    }
    ...
    public void lastSave()
    {
      lock (this.xmlDoc)
      {
        try
        {
          XmlDocument local_0 = new XmlDocument();
          local_0.Load(this.SetXml);
          if (!(this.xmlDoc.InnerText != local_0.InnerText) || !this.xmlDoc.InnerXml.Contains("SetInfo"))
            return;
