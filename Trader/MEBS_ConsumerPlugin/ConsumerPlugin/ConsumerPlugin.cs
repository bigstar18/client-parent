...
namespace ConsumerPlugin
{
	public class ConsumerPlugin : IPlugin
	{
		...
		public void Initialize()
		{
			...
			{
				PluginConfigInfo pluginConfigInfo = (PluginConfigInfo)this.myHost.HtConfigInfo[this.myName];
		...
		private void GetCommunicationUrl(int curServer)
		{
			...
			this.tradeLibrary.IsConsumerPlugin = true;
			...
			if (this.CheckUser(ref info))
			{
				if (this.myDisplayType == DisplayTypes.IEDialog)
				{
					text = string.Concat(new object[]
					{
						...
						"&userID=",
						this.myHost.SysLogonInfo.TraderID
					});
