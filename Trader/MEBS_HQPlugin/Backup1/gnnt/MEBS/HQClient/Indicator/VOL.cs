...
// Type: Gnnt.MEBS.HQClient.gnnt.MEBS.HQClient.Indicator.VOL
...
namespace Gnnt.MEBS.HQClient.gnnt.MEBS.HQClient.Indicator
{
  internal class VOL : IndicatorBase
  {
    ...
    private void GetMaxMin()
    {
      ...
      {
        if ((double) this.m_kData[index].totalAmount > (double) this.m_max)
          this.m_max = (float) this.m_kData[index].totalAmount;
      }
    }
    ...
    private void DrawVolume(Graphics g)
    {
      ...
      for (int index = this.m_pos.m_Begin; index <= this.m_pos.m_End; ++index)
      {
        int y = this.m_rc.Y + this.m_iTextH + (int) (((double) this.m_max - (double) this.m_kData[index].totalAmount) / (double) num3);
    ...
    private void AverageVolume(int iIndex)
    {
      ...
      for (int index = 0; index < num1 - 1; ++index)
        num3 += (double) this.m_kData[index].totalAmount;
      for (int index = num1 - 1; index < this.m_kData.Length; ++index)
      {
        num3 = num3 - (double) num2 + (double) this.m_kData[index].totalAmount;
        numArray[index] = (float) num3 / (float) num1;
        num2 = (float) this.m_kData[index - num1 + 1].totalAmount;
      }
    }
  }
}
