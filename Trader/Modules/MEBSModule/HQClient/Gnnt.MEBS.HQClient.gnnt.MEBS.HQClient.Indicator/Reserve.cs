...
namespace Gnnt.MEBS.HQClient.gnnt.MEBS.HQClient.Indicator
{
	internal class Reserve : IndicatorBase
	{
		...
		public Reserve(IndicatorPos pos, int Precision) : base(pos, Precision)
		{
		...
		private void GetMaxMin()
		{
			...
			{
				if ((float)this.m_kData[i].reserveCount > this.m_max)
				{
					this.m_max = (float)this.m_kData[i].reserveCount;
				}
			}
		}
		private void DrawVolume(Graphics g)
		{
			...
			for (int i = this.m_pos.m_Begin; i <= this.m_pos.m_End; i++)
			{
				int num4 = this.m_rc.Y + this.m_iTextH + (int)((this.m_max - (float)this.m_kData[i].reserveCount) / num3);
		...
		private void AverageVolume(int iIndex)
		{
			...
			{
				num3 += (double)this.m_kData[i].reserveCount;
			}
			for (int i = num - 1; i < this.m_kData.Length; i++)
			{
				...
				num2 = (float)this.m_kData[i - num + 1].reserveCount;
			}
		}
	}
}
