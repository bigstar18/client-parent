...
namespace Gnnt.MEBS.HQClient.gnnt.MEBS.HQClient
{
	public class Draw_MinLine : IDisposable
	{
		...
		internal void Paint(Graphics g, Rectangle rc, ProductData product)
		{
			...
			{
				Logger.wirte(3, "Draw_MinLine-Paint异常：" + ex.Message);
			}
		}
				...
				using (Graphics graphics = Graphics.FromImage(bitmap))
				{
					...
					if (this.m_iPos >= 0 && this.m_iPos < this.iNum)
					{
						...
						if (minDataVO != null)
						{
							...
							else if (minDataVO.curPrice < this.m_product.realData.yesterBalancePrice)
							{
						...
						if (minDataVO != null)
						{
							...
							else if (minDataVO.averPrice < this.m_product.realData.yesterBalancePrice)
							{
								solidBrush.Color = SetInfo.RHColor.clDecrease;
							}
							else
					...
					if (this.m_iPosY >= 0)
					{
						...
						using (Bitmap bitmap = new Bitmap(this.rcYCoordinate.Width, this.rcYCoordinate.Height))
						{
							using (Graphics graphics = Graphics.FromImage(bitmap))
							{
								...
								if (this.m_iPos < this.m_product.aMinLine.Count)
								{
									minDataVO = (MinDataVO)this.m_product.aMinLine[this.m_iPos];
								}
								string s = "";
								string format = "f" + this.m_iPrecision.ToString();
								if (this.m_iPosY >= this.m_rcPrice.Y && this.m_iPosY <= this.m_rcPrice.Y + this.m_rcPrice.Height)
								{
									if (minDataVO != null && this.m_iPosY == this.GetYFromPrice(minDataVO.curPrice))
									{
										s = minDataVO.curPrice.ToString(format);
									}
									else
									{
										s = this.GetPriceFromY(this.m_iPosY).ToString(format);
									}
								}
		...
		private void DrawGrid(Graphics g, Rectangle rc)
		{
			...
			for (int num13 = 0; num13 <= this.iPriceGridNum; num13++)
			{
				...
				else if (yesterBalancePrice > num14)
				{
					solidBrush.Color = SetInfo.RHColor.clDecrease;
				}
				else
		...
		private float GetPriceFromY(int y)
		{
			...
			return (float)Math.Round((double)(num / (float)this.m_rcPrice.Height), this.m_iPrecision) + this.m_minPrice;
		}
		...
		internal bool KeyPressed(KeyEventArgs e)
		{
			...
			if (keyData != Keys.Escape)
			{
				...
				case Keys.Left:
					...
					else if (this.m_iPos == -1 && this.iNum > 0)
					{
						this.isDrawCursor = true;
						this.DrawCursor(this.iNum - 1);
						this.DrawLabel();
