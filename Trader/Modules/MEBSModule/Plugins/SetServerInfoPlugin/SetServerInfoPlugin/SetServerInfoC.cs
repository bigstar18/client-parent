...
namespace SetServerInfoPlugin
{
	public class SetServerInfoC : UserControl
	{
		...
		private void LoadData()
		{
			...
			{
				this.strText = xmlNode.SelectSingleNode("Text").InnerText;
			}
			if (xmlNode.SelectSingleNode("CurTelecomServer") != null)
			{
				this.strCurTelecomServer = xmlNode.SelectSingleNode("CurTelecomServer").InnerText;
			}
			if (xmlNode.SelectSingleNode("CurNetcomServer") != null)
			{
				this.strCurNetcomServer = xmlNode.SelectSingleNode("CurNetcomServer").InnerText;
			}
			...
			XmlNode xmlNode4 = xmlNode.SelectSingleNode("AllNetcomServer");
			if (xmlNode2 != null)
			{
		...
		public void Initialize()
		{
			...
			groupBox.Text = ServerSetPlugin.strServerNameSec;
			...
			string @string = ServerSetPlugin.SysResourceManager.GetString("PluginStr_AutoSelect");
			...
			for (int j = 0; j < this.netcomServerList.Count; j++)
			{
				...
				string string5 = ServerSetPlugin.SysResourceManager.GetString("PluginStr_IpAddress");
				label4.Text = string5 + " ";
				...
				string string6 = ServerSetPlugin.SysResourceManager.GetString("PluginStr_StrPort");
				label5.Text = " " + string6 + " ";
				...
				if (this.serverType == 1)
				{
					...
					string string7 = ServerSetPlugin.SysResourceManager.GetString("PluginStr_HttpPort");
					label6.Text = string7;
		...
		private void tbPort_KeyPress(object sender, KeyPressEventArgs e)
		{
			...
			{
				string @string = ServerSetPlugin.SysResourceManager.GetString("PluginStr_PortNoZero");
		...
		public bool UpdateConfigInfo(string key, string value)
		{
			...
			if (xmlNode.SelectSingleNode(key) != null)
			{
				...
				string str = this.myXmlDoc.BaseURI.Substring(this.myXmlDoc.BaseURI.LastIndexOf("/"));
				this.myXmlDoc.Save(this.myXmlPath + str);
				result = true;
			}
			return result;
		}
			...
			if (xmlNode2 != null)
			{
				...
				foreach (XmlNode xmlNode4 in childNodes2)
				{
					...
					if (xmlElement2.SelectSingleNode("ServerName") != null && xmlElement2.SelectSingleNode("ServerName").InnerText.Equals(serverName) && xmlElement2.SelectSingleNode(key) != null)
					{
						...
						string str2 = this.myXmlDoc.BaseURI.Substring(this.myXmlDoc.BaseURI.LastIndexOf("/"));
						this.myXmlDoc.Save(this.myXmlPath + str2);
						flag = true;
						bool result = flag;
						return result;
					}
