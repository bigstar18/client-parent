...
namespace SysFrame.Gnnt.Common.Library
{
	public class ThreadStartAddNodes
	{
		...
		public static Font font = new Font("宋体", 12f, FontStyle.Regular);
		...
		public ThreadStartAddNodes(ToolStripItemCollection toolStripItem, string xmlPath)
		{
			...
			{
				Logger.wirte(3, "初始化资源文件异常：" + ex.Message);
			}
		}
		...
		private void populateTreeControl(XmlNode document, ToolStripItemCollection nodes)
		{
			...
			foreach (XmlNode xmlNode in document.ChildNodes)
			{
				...
				else
				{
					...
					if (xmlNode.Attributes != null && xmlNode.Attributes["Plugins"] != null)
					{
						...
						if (value.Length > 0)
						{
							...
							for (int i = 0; i < array.Length; i++)
							{
								...
								{
									list.Add(availablePluginInfo.get_Instance());
								}
							}
					...
					if (list2.Count > 0 && list2[0].get_Name() != "ServerSet" && list2[0].get_Name() != "MixAccount")
					{
						bool flag = false;
		...
		public void addNodeChild(XmlNode document, ToolStripItemCollection nodes)
		{
			try
			{
				...
				toolStripItem.AutoSize = false;
				string @string = Global.Modules.get_Plugins().get_MEBS_ResourceManager().GetString("TradeStr_SysForm_Logout");
				Global.Modules.get_Plugins().get_MEBS_ResourceManager().GetString("TradeStr_SysForm_LogoutSys");
				toolStripItem.Size = ThreadStartAddNodes.new_child_size;
				toolStripItem.DisplayStyle = ToolStripItemDisplayStyle.ImageAndText;
				toolStripItem.Margin = new Padding(0, 0, 0, 0);
				...
				string string2 = Global.Modules.get_Plugins().get_MEBS_ResourceManager().GetString("TradeStr_SysForm_ChangePws");
			...
			{
				Logger.wirte(3, "addNodeChild异常：" + ex.Message);
			}
		}
		public void addLoginChild(XmlNode document, ToolStripItemCollection nodes)
		{
			...
			toolStripItem.Margin = new Padding(0, 0, 0, 0);
			string text = "";
			if (Global.Modules.get_Plugins().get_MEBS_ResourceManager().GetString("TradeStr_SysForm_login") != null)
			{
				text = Global.Modules.get_Plugins().get_MEBS_ResourceManager().GetString("TradeStr_SysForm_login");
			}
			if (text == "")
			{
				text = "登  录";
