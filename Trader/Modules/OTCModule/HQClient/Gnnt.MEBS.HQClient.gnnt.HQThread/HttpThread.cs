	...
	public class HttpThread : MYThread
	{
		...
		protected override void run()
		{
			try
			{
				...
				{
					this.GetCodeList();
					Logger.wirte(1, "**********" + this.threadName + "取码表结束！！！**********");
				}
				else if (2 == this.iType)
				{
					...
					{
						this.GetMarketList();
						Logger.wirte(1, "**********" + this.threadName + "取市场列表结束！！！**********");
					}
					else
					{
						this.GetXMLClass();
						Logger.wirte(1, "**********" + this.threadName + "取商品分类信息结束！！！**********");
					}
				}
				else
				{
			...
			{
				Logger.wirte(3, "HttpThread-run异常：" + ex.Message);
			}
		}
			...
			while (this.m_client != null && !this.blnIsStopped && !flag)
			{
				...
				try
				{
					...
					for (int i = 0; i < productInfos.Length; i++)
					{
						...
						{
							Logger.wirte(2, "HTTP取码表出现重复数据，将出现Hashtable重复键加入！商品代码" + productInfos[i].code);
						}
		...
		private void GetXMLClass()
		{
			...
			while (this.m_client != null && !this.blnIsStopped && !flag)
			{
				...
				{
					Logger.wirte(3, "下载市场分类XML文件" + ex.Message);
		...
		public void RemakeButton()
		{
			...
			{
				Logger.wirte(3, "RemakeButton异常：" + ex.Message);
			}
		}
		...
		public static KLineData[] GetHistoryData(string url)
		{
			...
			{
				Logger.wirte(3, "GetHistoryData异常：" + ex.Message);
				result = null;
			}
			return result;
		}
		public static KLineData[] GetLocalHistoryData(string url)
		{
			...
			{
				Logger.wirte(3, "GetLocalHistoryData异常：" + ex.Message);
				result = null;
			}
			return result;
		}
		private void GetDayLine(Packet_HttpRequest request)
		{
			...
			{
				Logger.wirte(3, "UriFormatException" + ex.ToString());
			}
			catch (IOException ex2)
			{
				Logger.wirte(3, "IOException" + ex2.ToString());
			}
			catch (Exception ex3)
			{
				Logger.wirte(3, "Exception" + ex3.ToString());
			}
		}
		...
		public bool DownloadFile(string StrUrl, string StrFileName)
		{
			...
			catch (Exception ex)
			{
				...
				Logger.wirte(3, "下载文件异常：" + ex.Message);
				result = false;
			}
			return result;
		}
		private bool UnZipFile(string fileName)
		{
				...
				using (FileStream fileStream = new FileStream(fileName, FileMode.Open))
				{
					...
					using (FileStream fileStream2 = File.Create(path))
					{
						using (GZipStream gZipStream = new GZipStream(fileStream, CompressionMode.Decompress))
						{
							...
							Logger.wirte(1, "解压文件：" + fileName + "成功。");
						}
					}
				}
			...
			{
				Logger.wirte(3, "解压缩文件：" + fileName + "出错：" + ex.Message);
				result = false;
			}
			return result;
		}
