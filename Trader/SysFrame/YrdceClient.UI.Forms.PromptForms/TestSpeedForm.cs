...
namespace YrdceClient.UI.Forms.PromptForms
{
	public class TestSpeedForm : Form
	{
		...
		{
			this.pnlMain.BackgroundImage = (Image)Global.Modules.Plugins.MEBS_ResourceManager.GetObject("TradeImg_Skin1");
		...
		public void AddNetInfoNode()
		{
			foreach (DictionaryEntry dictionaryEntry in Global.Modules.Plugins.LoginPluginsHashtable)
			{
				...
				if (OperationManager.GetInstance().stripButtonOperation.curLoginPluginName == Global.Modules.Plugins.LoginPluginsHashtable[toolStripItem.Name].ToString())
				{
		...
		{
			Hashtable section = Global.Modules.Plugins.ConfigurationInfo.getSection("Systems");
		...
		{
			OperationManager.GetInstance().stripButtonOperation.curLoginPluginName = Global.Modules.Plugins.AvailablePlugins.Find(Global.Modules.Plugins.LoginPluginsHashtable[name].ToString()).Instance.Name;
