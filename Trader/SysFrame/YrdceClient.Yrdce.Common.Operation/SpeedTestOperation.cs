...
namespace YrdceClient.Yrdce.Common.Operation
{
	public class SpeedTestOperation
	{
		...
		private string TelecomStr = Global.Modules.Plugins.MEBS_ResourceManager.GetString("TradeStr_LoginForm_Telecom");
		private string UnicomStr = Global.Modules.Plugins.MEBS_ResourceManager.GetString("TradeStr_LoginForm_Unicom");
		...
		public void serverSpeedDefault()
		{
			...
			try
			{
				dataSet.ReadXml(this.myPlugin.ConfigFileName);
			}
			catch (Exception)
			{
				string @string = Global.Modules.Plugins.MEBS_ResourceManager.GetString("TradeStr_LoginForm_getServerListError");
		...
		{
			PluginConfigInfo pluginConfigInfo = (PluginConfigInfo)this.myPlugin.Host.HtConfigInfo[this.myPlugin.Name];
			XmlElement xmlElement;
			if (this.myPlugin.Name.Contains("OTC"))
			{
				...
				for (int i = 0; i < count; i++)
				{
					...
					if (text.IndexOf(this.TelecomStr) != -1)
					{
						if (num2 == -1)
						{
							...
							if (a == this.myPlugin.IpAddress && num3 == this.myPlugin.Port)
							{
				...
				for (int j = 0; j < count; j++)
				{
					...
					if (text2.IndexOf(this.UnicomStr) != -1)
					{
						if (num2 == -1)
						{
							...
							if (a2 == this.myPlugin.IpAddress && num4 == this.myPlugin.Port)
							{
		...
		private void ServerSpeed(object obj)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (num >= 0 && num <= 50)
				{
					string @string = Global.Modules.Plugins.MEBS_ResourceManager.GetString("TradeStr_LoginForm_faster");
					text += @string;
				}
				else if (num > 50 && num <= 200)
				{
					string string2 = Global.Modules.Plugins.MEBS_ResourceManager.GetString("TradeStr_LoginForm_fast");
					text += string2;
				}
				else if (num > 200 && num <= 500)
				{
					string string3 = Global.Modules.Plugins.MEBS_ResourceManager.GetString("TradeStr_LoginForm_slow");
					text += string3;
				}
				else if (num > 500 && num <= 2000)
				{
					string string4 = Global.Modules.Plugins.MEBS_ResourceManager.GetString("TradeStr_LoginForm_slower");
					text += string4;
				}
				else if (num > 2000)
				{
					string string5 = Global.Modules.Plugins.MEBS_ResourceManager.GetString("TradeStr_LoginForm_Slowest");
					text += string5;
				}
				else
				{
					string string6 = Global.Modules.Plugins.MEBS_ResourceManager.GetString("TradeStr_LoginForm_UnableConnect");
					text += string6;
				}
		...
		public void setServerInfo()
		{
			...
			this.myPlugin.Host.ConfigurationInfo.updateValue("Systems", "CurServer", string.Concat(num));
			int count = this.listBoxServerSpeed.Items.Count;
			int num2 = -1;
			int num3 = -1;
			for (int i = 0; i < count; i++)
			...
			xmlDocument.Load(this.myPlugin.ConfigFileName);
			...
			xmlDocument.Save(this.myPlugin.ConfigFileName);
			PluginConfigInfo pluginConfigInfo = (PluginConfigInfo)this.myPlugin.Host.HtConfigInfo[this.myPlugin.Name];
		...
		{
			foreach (AvailablePluginInfo availablePluginInfo in Global.Modules.Plugins.AvailablePlugins)
			{
				if (availablePluginInfo.Instance != this.myPlugin)
				{
