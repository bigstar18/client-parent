...
using TPME.Log;
namespace YrdceClient.Yrdce.UI.Forms.ContainerManager
{
	public class ContainerManage
	{
		...
		public BackgroundWorker LoginProgressWorker = new BackgroundWorker();
		private Panel TopPanel = new Panel();
		private Panel HQPanel = new Panel();
		private Panel TradePanel = new Panel();
        private Splitter splitterHQTrade = new Splitter();
		private int TopControlHeight = 35;//主窗体菜单栏高度
		private int PanelTradeHeight = 280;
		...
		public ContainerManage(Control control)
		{
			...
			this.operationManager.YrdceClientOperation.lockTradeFormEventCallBack = new YrdceClientOperation.LockTradeFormEventCallBack(this.LockTradeForm);
			this.operationManager.YrdceClientOperation.floatTradeFormEventCallBack = new YrdceClientOperation.FloatTradeFormEventCallBack(this.FloatTradeForm);
			this.operationManager.YrdceClientOperation.logOutTradeFormEventCallBack = new YrdceClientOperation.LogOutTradeFormEventCallBack(this.operationManager.stripButtonOperation.Logout);//宋
		...
		private void FillTopControl()
		{
            ...
            stripButtonControl.BackgroundImage = Image.FromFile("images\\Menu.jpg");
		...
		private void FillHQTrade(PanelLoad panel, FormandPlugin temp)
		{
			...
			case PanelLoad.HQPanelLoad:
				if (!temp.plugin.Name.EndsWith("HQSystem"))
				{
					this.FillHQPanel(temp);
					return;
				}
				if (this.operationManager.PluginNameList.Contains(temp.plugin.Name) || !this.operationManager.isLogin)
				{
					this.FillHQPanel(temp);
					return;
				}
				...
				this.operationManager.PluginNameList.Add(temp.plugin.Name);
				return;
			case PanelLoad.TradePanelLoad:
				if (this.operationManager.PluginNameList.Contains(temp.plugin.Name))
				{
					if (temp.plugin.Name == "OTC_Trade")
					{
		...
		public void FillHQPanel(FormandPlugin formAndPlugin)
		{
			Form form = null;
			...
			if (form == null)
			{
				this.HQPanel.BackgroundImage = (Image)TimeImage.MainBG;
			...
			else
			{
				...
				if (this.hqGrid != null && formAndPlugin.plugin != null && this.operationManager.isLogin)
				{
					...
					if (formAndPlugin.plugin.Name.Equals("OTC_HQSystem") || formAndPlugin.plugin.Name.Equals("OTC_Trade"))
					{
		...
		private void FillTradePanel(Form tradeForm)
		{
			...
			if (!this.operationManager.YrdceClientOperation.isLockTrade)
			{
		...
		{
			this.HQPanel.Controls.Clear();
			//this.HQPanel.BackgroundImage = (Image)Global.Modules.Plugins.MEBS_ResourceManager.GetObject("HQImg_main");
			this.HQPanel.BackgroundImageLayout = ImageLayout.Stretch;
			this.TradePanel.Controls.Clear();
            this.TopPanel.Height = this.TopControlHeight;
			this.TradePanel.Visible = false;
		}
		public void splitterHQTrade_SplitterMoved(object sender, SplitterEventArgs e)
		{
			this.PanelTradeHeight = this.TradePanel.Height;
		}
		private void TradePanel_Paint(object sender, PaintEventArgs e)
		{
			if (this.curForm != null && this.operationManager.YrdceClientOperation.isLockTrade)
			{
		...
		private void LockTradeForm(bool isLockTrade)
		{
			if (!isLockTrade)
			{
				...
				{
					this.operationManager.YrdceClientOperation.isLockTrade = true;
					if (this.operationManager.FormStyle == 8 && this.operationManager.stripButtonOperation.curLoginPluginName.Contains("MEBS"))
					{
		...
		private void FloatTradeForm(bool isFloat)
		{
			if (!isFloat)
			{
				...
				this.operationManager.YrdceClientOperation.isFloatTrade = true;
				return;
			}
			if (this.curForm != null)
			{
			...
			this.operationManager.YrdceClientOperation.isFloatTrade = false;
		}
		...
		public void renewPanel2()
		{
			if (this.operationManager.YrdceClientOperation.isLockTrade)
			{
				this.operationManager.YrdceClientOperation.isLockTrade = false;
				if (this.operationManager.FormStyle == 8 && this.operationManager.stripButtonOperation.curLoginPluginName.Contains("MEBS"))
				{
					this.HQPanel.Height -= this.PanelTradeHeight + this.height - 25;
					this.TradePanel.Height = this.PanelTradeHeight + this.height;
		...
		public bool PointInFences(Point pnt1, Point[] fencePnts)
		{
			int num = 0;
			int num2 = 0;
		...
		public void ShowProgress(string strInfo, int val)
		{
			try
			{
		...
		private void LoginProgressWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
		{
			...
			try
			{
				...
				if (form != null && plugin.DisplayType == DisplayTypes.panel2)
				{
					if (form.Tag != null)
					{
